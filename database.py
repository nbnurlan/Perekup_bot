...
def filter_new_ad_ids(ad_ids: list[str]) -> set[str]:
    """
    Bolt optimization ⚡: Batch query to check multiple ad IDs in a single query.
    Reduces N DB connections/queries to 1 single SELECT query with parameterized IN clause.
    Returns a set containing only the ad IDs that are new (not yet seen).
    """
    if not ad_ids:
        return set()

    # Deduplicate candidate IDs preserving unique values
...
def save_ads(ad_ids: list[str]) -> None:
    """
    Bolt optimization ⚡: Batch insert multiple ad IDs in a single transaction using executemany.
    Avoids opening multiple DB connections and executing multiple commit operations.
    """
    if not ad_ids:
        return

    # Prepare list of tuples for executemany
    records = [(ad_id,) for ad_id in set(ad_ids)]
